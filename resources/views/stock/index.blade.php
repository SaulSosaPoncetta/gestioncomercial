    ...
    <div class="card-body py-2">
        <form method="GET" action="{{ route('stock.index') }}" class="row g-2">
            ...
            <div class="col-md-auto">
                ...
                <a href="{{ route('stock.index') }}" class="btn btn-outline-secondary"><i class="bi bi-x-lg"></i></a>
                @endif
            </div>
        </form>
    </div>
        ...
        <table class="table table-hover mb-0 align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-end">{{ rtrim(rtrim(number_format($item->cantidad, 3, '.', ''), '0'), '.') }}</td>
                    <td class="text-end">{{ $item->producto->stock_minimo ?? 0 }}</td>
                    <td>
                        @if($item->producto && $item->cantidad <= $item->producto->stock_minimo)
                            <span class="badge bg-danger"><i class="bi bi-exclamation-triangle me-1"></i>Bajo mínimo</span>
                        @else
                ...
                @empty
                <tr><td colspan="6" class="text-center text-muted py-4">No hay stock cargado todavía.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
